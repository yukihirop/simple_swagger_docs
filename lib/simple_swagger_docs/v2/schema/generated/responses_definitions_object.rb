...
require_relative 'patterned/name/name_response_object_patterned_object'
  ...
  module V2
    module Schema
      class ResponsesDefinitionsObject
        ...
        attr_accessor :responses_definitions_object
        ...
        def definitions
          if openapi_data_at_responses_definitions_object&.fetch(:definitions, nil).is_a?(Hash)
            return @responses_definitions_object[:definitions] unless @responses_definitions_object[:definitions].nil?
            definitions_content = []
            openapi_data_at_responses_definitions_object[:definitions].each do |key, value|
              data = { name: key, content: value }
              definitions_content << NameResponseObjectPatternedObject.new(data)
            end
            @responses_definitions_object[:definitions] = definitions_content
          else
            @responses_definitions_object[:definitions] || openapi_data_at_responses_definitions_object[:definitions]
          end
        end
        ...
        def openapi_data_at_responses_definitions_object
