...
module SimpleSwaggerDocs
  module V2
    module Schema
      class NameSchemaObjectPatternedObject
        include NodeUpdatable
        include DocsMethods

        attr_accessor :name
        attr_accessor :content
        attr_accessor :name_schema_object_patterned_object

        def initialize(openapi_data = {})
          check_initialize_args(openapi_data)
          self.name = openapi_data[:name]
          @openapi_data = openapi_data
          @name_schema_object_patterned_object ||= {
            name: nil,
            content: nil,
        }
        end
        ...
        def content
          if openapi_data_at_name_schema_object_patterned_object&.fetch(:content, nil).is_a?(Hash)
            return @name_schema_object_patterned_object[:content] unless @name_schema_object_patterned_object[:content].nil?
            data = @openapi_data[:content]
            if SchemaObject.have_patterned_methods?
              data = { content: data }
            end
            @name_schema_object_patterned_object[:content] = SchemaObject.new(data)
          else
            @name_schema_object_patterned_object[:content] || openapi_data_at_name_schema_object_patterned_object[:content]
          end
        end
        ...
        def openapi_data_at_name_schema_object_patterned_object
