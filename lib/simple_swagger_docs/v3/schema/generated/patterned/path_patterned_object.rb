  ...
  module V3
    module Schema
      class PathPatternedObject
        ...
        def content
          if openapi_data_at_path_patterned_object&.fetch(:content, nil).is_a?(Hash)
            return @path_patterned_object[:content] unless @path_patterned_object[:content].nil?
            data = @openapi_data[:content]
            if PathItemObject.have_patterned_methods?
              data = { content: data }
            end
            @path_patterned_object[:content] = PathItemObject.new(data)
          else
            @path_patterned_object[:content] || openapi_data_at_path_patterned_object[:content]
          end
        end
        ...
        def openapi_data_at_path_patterned_object
