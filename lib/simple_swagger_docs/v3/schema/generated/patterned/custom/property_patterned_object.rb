  ...
  module V3
    module Schema
      class PropertyPatternedObject
        ...
        def content
          if openapi_data_at_property_patterned_object&.fetch(:content, nil).is_a?(Hash)
            return @property_patterned_object[:content] unless @property_patterned_object[:content].nil?
            data = @openapi_data[:content]
            if SchemaObject.have_patterned_methods?
              data = { content: data }
            end
            @property_patterned_object[:content] = SchemaObject.new(data)
          else
            @property_patterned_object[:content] || openapi_data_at_property_patterned_object[:content]
          end
        end
        ...
        def openapi_data_at_property_patterned_object
